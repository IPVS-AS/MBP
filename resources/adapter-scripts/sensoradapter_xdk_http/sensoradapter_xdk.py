...
def main(argv):
   ...
   publisher.sendMessage (topic_pub, json.dumps({"component": component.upper(), "id": component_id, "value": "%f" % (float(0))}))

   @app.route('/post', methods=['POST'])
   def result():
      request_data_raw = request.data.decode('utf-8', 'ignore')[:-1]
      request_data = ""

      for letter in request_data_raw:
         request_data += letter
         if(letter == "}"):
            break

      parsed_json = json.loads(request_data)
      value = parsed_json['light']
