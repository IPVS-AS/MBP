...
public class KeyPairValidator implements Validator {
	 ...
	 * However, this validator can only be applied to key pair objects.
	 *
	...
	/**
	 * Validates the fields of a key pair object and adds error messages to the
	...
	public void validate(Object o, Errors errors) {
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "name", "keypair.name.empty", "The name must not be empty.");

		// Private key is mandatory
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "privateKey", "keypair.privateKey.empty",
				"No private key provided.");

		// Check if name is unique
		if (keyPairRepository.existsByName(keyPair.getName())) {
			errors.rejectValue("name", "keypair.name.duplicate", "The name is already registered.");
		}
	}
}
