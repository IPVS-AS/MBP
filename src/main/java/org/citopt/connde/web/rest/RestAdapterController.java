...
public class RestAdapterController {
    ...
    @GetMapping(path = "/{adapterId}", consumes = MediaType.APPLICATION_JSON_VALUE, produces = "application/hal+json")
    ...
    public ResponseEntity<EntityModel<Adapter>> one(
    	...
    	Adapter adapter = userEntityService.getForIdWithPolicyCheck(adapterRepository, adapterId, ACAccessType.READ, accessRequest);
    ...
    public ResponseEntity<EntityModel<Adapter>> create(@PathVariable("adapterId") String adapterId, @ApiParam(value = "Page parameters", required = true) Pageable pageable, @RequestBody Adapter adapter) {
    ...
    @ApiResponses({ @ApiResponse(code = 204, message = "Success!"),
    		...
    		@ApiResponse(code = 404, message = "Adapter or requesting user not found!") })
    public ResponseEntity<Void> delete(@PathVariable("adapterId") String adapterId, @Valid @RequestBody ACAccessRequest<?> accessRequest) {
    	// Delete the adapter (includes access-control) 
    	userEntityService.deleteWithPolicyCheck(adapterRepository, adapterId, accessRequest);
