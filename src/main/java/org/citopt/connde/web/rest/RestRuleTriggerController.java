...
public class RestRuleTriggerController {
    ...
    @GetMapping(path = "/{ruleTriggerId}", consumes = MediaType.APPLICATION_JSON_VALUE, produces = "application/hal+json")
    ...
    public ResponseEntity<EntityModel<RuleTrigger>> one(
    	...
    	RuleTrigger ruleTrigger = userEntityService.getForIdWithPolicyCheck(ruleTriggerRepository, ruleTriggerId, ACAccessType.READ, accessRequest);
    ...
    public ResponseEntity<EntityModel<RuleTrigger>> create(
    		@PathVariable("ruleTriggerId") String ruleTriggerId, @ApiParam(value = "Page parameters", required = true) Pageable pageable,
    		@RequestBody RuleTrigger ruleTrigger) {
    ...
    public ResponseEntity<Void> delete(
    		@PathVariable("ruleTriggerId") String ruleTriggerId,
    		@Valid @RequestBody ACAccessRequest<?> accessRequest) {
    	// Delete the rule trigger (includes access-control) 
    	userEntityService.deleteWithPolicyCheck(ruleTriggerRepository, ruleTriggerId, accessRequest);
