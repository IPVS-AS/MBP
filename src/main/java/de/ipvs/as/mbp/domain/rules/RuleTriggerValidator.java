...
public class RuleTriggerValidator implements Validator {
	 ...
	 * However, this validator can only be applied to rule trigger objects.
	 *
	...
	/**
	 * Validates the fields of a rule trigger object and adds error messages to the
	...
	public void validate(Object o, Errors errors) {
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "name", "ruleTrigger.name.empty",
				"The name must not be empty.");

		// Check if name is unique
		if (ruleTriggerRepository.existsByName(ruleTrigger.getName())) {
			errors.rejectValue("name", "ruleTrigger.name.duplicate", "The name is already registered.");
		}

		// Check if query string was provided (mandatory)
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "query", "ruleTrigger.query.empty",
