...
public class RuleValidator implements Validator {
	 ...
	 * However, this validator can only be applied to rule objects.
	 *
	...
	/**
	 * Validates the fields of a rule object and adds error messages to the errors
	 * object accordingly.
	 *
	...
	public void validate(Object o, Errors errors) {
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "name", "rule.name.empty", "The name must not be empty.");

		// Check if name is unique
		if (ruleRepository.existsByName(rule.getName())) {
			errors.rejectValue("name", "rule.name.duplicate", "The name is already registered.");
		}

		// Check if a rule trigger was provided
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "trigger", "rule.trigger.empty",
		...
		if ((ruleActions == null) || ruleActions.isEmpty()) {
			errors.rejectValue("actions", "rule.actions.empty", "At least one rule action needs to be selected.");
		}
	}
}
