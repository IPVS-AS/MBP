...
public class EntityTypeCreateValidator implements ICreateValidator<EntityType> {
    ...
    public void validateCreatable(EntityType entity) {
        ...
        if ((size < MIN_ALLOWED_ICON_SIZE) || (size > MAX_ALLOWED_ICON_SIZE)) {
            exception.addInvalidField("icon", "The icon is of an invalid size.");
            throw exception;
        }
        ...
        //Check for null
        if (iconImage == null) {
            exception.addInvalidField("icon", "Invalid icon provided.");
            throw exception;
        }
        ...
        if ((iconImage.getWidth() < MIN_WIDTH) ||
            ...
            exception.addInvalidField("icon", "The icon must be a square and between " + MIN_WIDTH +
