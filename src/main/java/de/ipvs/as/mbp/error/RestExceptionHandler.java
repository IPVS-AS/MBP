...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleValidationError(EntityValidationException e) {
        ApiError error;

        //Decide about status code
        HttpStatus status = e.hasInvalidFields() ? HttpStatus.BAD_REQUEST : HttpStatus.INTERNAL_SERVER_ERROR;

        //Check if there were invalid fies
        if (e.hasInvalidFields()) {
            error = new ApiError(status, LocalDateTime.now(), "One or more parameters are invalid or missing.", e.getInvalidFields());
        } else {
