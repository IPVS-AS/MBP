...
public class ScriptDeployerTest {
    ...
    static String sensorbase = "import logging as log\n"
            + "import abc\n"
            + "import time\n"
            + "import collections\n"
            + "import argparse\n"
    ...
    static String sensorstub = "import sys\n"
            + "from random import randint\n"
    ...
    static String service = "description \"Sensor Stub\"\n"
            + "author \"CitOpt\"\n"
            + "\n"
            ...
            + "\n"
            + "chdir %dir%\n"
            + "exec python3 sensorstub.py -i %id% -t sensor/%id% -u %mqtturl%\n"
            + "respawn";

    static MongoClient mc;
    static DB db;
    static DataService ds;
    static Sensor sensorreg;
    ...
    public static void setUpClass() throws UnknownHostException {
        mc = MongoUtils.getMongoClient();
        db = MongoUtils.getMongoDB(mc);
        ds = new DataService(MongoUtils.getDatastore(mc, "DataServiceTest"),
                MongoUtils.getMongoDB(mc));

        Device d = new Device("b8:27:eb:c4:ff:96");
        ds.storeDevice(d);

        Script script = new Script("anotherstub", "sends random values");
        script.setService(service);
        script.addScript("sensorbase.py", sensorbase);
        script.addScript("sensorstub.py", sensorstub);
        ds.storeScript(script);

        sensorreg = new Sensor(d, new ArrayList<>(), script);
        ds.storeSensor(sensorreg);
    }
    ...
    public void testDeployScript() throws Exception {
        ...
        String mac = "b8:27:eb:c4:ff:96";
        String mqtt = "129.69.209.80";
        String url = "129.69.209.86";
        Integer port = 22;
        String user = "pi";

        //Sensor sensor = ds.querySensor().get();
        Sensor sensor = sensorreg;
        
        ARP arp = new ARP(db);
        //url = arp.getIp(sensor.getDevice().getMacAddress());

        if (url != null) {
            System.out.println(url);
            ScriptDeployer instance = new ScriptDeployer();

            instance.deployScript(sensor.getId().toString(),
                    sensor.getPinSet(), sensor.getScript(), url, port, mqtt, user, key);
        } else {
            fail("mac not found in network");
        }
    }
